...
class QWE_Generator {
    ...
    public static function generate( $keyword, $keyword_type = 'longtail', $hint_category = '', $difficulty = 'beginner', $provider = null, $tool_name = '' ) {
        ...
        // Pass 2: E-E-A-T evaluation — revise if needed, reject if unsalvageable.
        // review_and_revise() returns:
        //   array  — use this article (passed or revised)
        //   null   — infrastructure failure (timeout/parse): fall back to Pass 1 draft
        //   false  — quality rejection (low scores): skip keyword entirely
        $final = self::review_and_revise( $article, $provider );
        if ( false === $final ) {
            self::log( "Pass 2 rejected article (quality) — skipping keyword: {$keyword}" );
            return false;
        }
        if ( null === $final ) {
            self::log( "Pass 2 infrastructure failure — publishing Pass 1 draft for: {$keyword}" );
            // $article stays as the Pass 1 draft.
        } else {
            $article = $final;
        }
...
3. INFORMATION RHYTHM — Exactly 2 paragraphs in the article may NOT directly solve a problem. They can be: an analogy, a brief comment reflecting on what was just explained, or an open-ended question that you leave without a definitive answer. These paragraphs give the article breathing room and make it feel human. All OTHER paragraphs must maintain high information density — every sentence teaches or moves the reader forward. Do NOT exceed 2 breathing paragraphs — more than 2 is filler.

=== NO-REPETITION RULE (MANDATORY) ===

Each piece of information may appear ONCE in the article. No exceptions:
- A limitation, gotcha, or constraint mentioned in one section must NOT appear again in another section (not even as a "reminder")
- Do NOT restate in a summary what was already said in the body
- Do NOT repeat a warning or caveat across multiple sections
- If two sections logically share information, choose which section owns it and keep it only there
- The FAQ section may reference earlier concepts but must NOT copy-paste or restate full explanations already given

Before finalizing, mentally scan each H2 section for duplicate content with other sections. If you find a repeated point, delete it from the less important location.

=== DEPTH OVER BREADTH ===

One section with real depth beats three sections of surface-level coverage. For every claim:
- Give the WHY, not just the WHAT ("the 50MB CSV limit exists because each row requires parsing overhead, not because of a file size cap")
- Give the actual consequence ("session expires mid-analysis? You lose all variables — re-upload your last saved CSV")
- Give the specific workaround when one exists, not "you can work around this"

Generic advice that applies to every tool in a category (e.g., "always back up your data") adds zero value. Either make it specific to THIS tool or cut it.
...
Every data point must be attributed. Write all attributions in LANGUAGE_PLACEHOLDER — do NOT mix in any other language:
- Official docs: "according to OpenAI's official documentation", "per Anthropic's pricing page"
- Official announcements: "in [Company]'s blog post", "in their March 2025 changelog"
- Academic papers: "according to [Author/Institution]'s research", "the GPT-4 technical report shows", "a [University] study found"
- Benchmarks: "per [Source]'s benchmark", "LMSYS Chatbot Arena ranks it at..."
- Community knowledge: "users on Reddit report", "a common workaround found in the community"
...
- Label sources honestly in the text using LANGUAGE_PLACEHOLDER only: "per the official docs", "community reports", "testing shows", etc. — never mix in another language
...
- Check that sources are labeled honestly in the text in the article's language — never in a foreign language
- Verify the article contains 3-5 genuinely useful insights, not just surface-level information
- REPETITION CHECK: Scan for any information that appears in more than one section. The same limitation, gotcha, warning, or fact must NOT appear twice. If found → REVISE: remove from the less important location
...
- RHYTHM fix: If < 2 breathing paragraphs, insert exactly 2 (analogy, reflection, or open question). If > 2, remove the extras down to exactly 2. If non-breathing paragraphs have fluff, cut it.
- REPETITION fix: Find any fact, limitation, or warning that appears in more than one section. Keep it in the most important location. Delete all other occurrences. The FAQ may reference earlier points briefly but may not re-explain them.
- DEPTH fix: For any section that only states WHAT without explaining WHY, add one sentence explaining the underlying reason. Replace generic advice that applies to any tool with advice specific to this exact tool.
...
PROMPT;
    ...
    private static function review_and_revise( $draft_article, $provider = null ) {
        ...
        $review_model = defined( 'QWE_CLAUDE_REVIEW_MODEL' ) ? QWE_CLAUDE_REVIEW_MODEL : null;
        self::log( 'Pass 2: Sending draft for E-E-A-T evaluation (model: ' . ( $review_model ?: QWE_CLAUDE_MODEL ) . ')' );

        $response = self::call_api( $system_prompt, $user_prompt, false, $provider, $review_model );

        if ( ! $response ) {
            // Infrastructure failure (timeout, network, API error) — NOT a quality decision.
            // Return null so the caller can fall back to the Pass 1 draft rather than
            // discarding a potentially good article due to a transient error.
            self::log( 'Pass 2 API call failed (infrastructure) — caller will fall back to Pass 1 draft' );
            return null;
        }

        // Parse using the same 5-method cascade as Pass 1.
        ...
        // Method 3: Brace-counting from the first '{'.
        if ( json_last_error() !== JSON_ERROR_NONE ) {
        ...
        // Method 4: Anchor to {"review" key, then brace-count (handles preamble text).
        if ( json_last_error() !== JSON_ERROR_NONE ) {
            if ( preg_match( '/(\{\s*"review"\s*:)/s', $response, $m, PREG_OFFSET_CAPTURE ) ) {
                $candidate = substr( $response, $m[1][1] );
                $json_str  = self::extract_json_object( $candidate );
                if ( $json_str ) {
                    $result = json_decode( $json_str, true );
                    if ( json_last_error() === JSON_ERROR_NONE ) {
                        self::log( 'Pass 2: JSON extracted via review-key anchor' );
                    }
                }
            }
        }

        // Method 5: Simple first/last brace fallback.
        if ( json_last_error() !== JSON_ERROR_NONE ) {
        ...
        if ( json_last_error() !== JSON_ERROR_NONE ) {
            ...
            // Parse failure is an infrastructure/model issue — fall back to Pass 1 draft.
            return null;
        }

        // Review data must exist.
        if ( ! isset( $result['review'] ) ) {
            self::log( 'Pass 2 response missing "review" key — infrastructure issue, falling back to Pass 1' );
            return null;
        }
        ...
        // Hard quality floor: if E-E-A-T average is below 65, reject outright.
        // Publishing a badly scored article is worse than skipping the keyword.
        $eeat_scores = array_filter( array(
            $r['experience_score']  ?? null,
            $r['expertise_score']   ?? null,
            $r['authority_score']   ?? null,
            $r['trust_score']       ?? null,
        ), 'is_numeric' );
        if ( count( $eeat_scores ) === 4 ) {
            $eeat_avg = array_sum( $eeat_scores ) / 4;
            if ( $eeat_avg < 65 ) {
                self::log( sprintf( 'Pass 2: E-E-A-T average %.1f < 65 — article rejected, keyword will be skipped', $eeat_avg ) );
                return false;
            }
        }
        ...
        if ( ! isset( $result['article'] ) ) {
            self::log( 'Pass 2: no revised article in response — keyword skipped' );
            return false;
        }
        ...
        foreach ( $required as $field ) {
            if ( empty( $revised[ $field ] ) ) {
                self::log( "Pass 2 revised article missing required field: {$field} — keyword skipped" );
                return false;
            }
        }
     ...
     */
    private static function call_claude_api( $system_prompt, $user_prompt, $use_web_search = false, $model = null ) {
        $api_key = QWE_CLAUDE_API_KEY;
        $model = $model ?: QWE_CLAUDE_MODEL;
        ...
        // Timeout: web search needs more time; Pass 2 sends large article content.
        // Pass 2 is identified by $use_web_search=false AND a large prompt, so give all
        // non-search calls 240 s to be safe (Opus review of a 2000-word article takes time).
        $timeout = $web_search_active ? 300 : 240;
     ...
     * @return string|false          Response text or false.
     */
    private static function call_api( $system_prompt, $user_prompt, $use_web_search = false, $provider = null, $model = null ) {
        if ( null === $provider ) {
            $ps = QWE_DB::get_provider_settings();
            $provider = $ps['provider'];
        }
        ...
        return self::call_claude_api( $system_prompt, $user_prompt, $use_web_search, $model );
    }
